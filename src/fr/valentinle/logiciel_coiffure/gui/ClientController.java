...
public class ClientController implements Initializable, DashboardListener, ClientListener {
	...
	SpinnerValueFactory<Integer> valueFactory = new SpinnerValueFactory.IntegerSpinnerValueFactory(0, Client.MAX_FIDELITY, this.client.getFidelity());
	...
	if (!this.familyName.getText().trim().isEmpty() && !this.name.getText().trim().isEmpty()) {
		...
		this.dashboard.addClient(this.client, false);
	...
	if (dashboard.clientExists(this.client)) {
	    ...
	    boolean wantCreateClient = this.confirmationDialog("Le client n'est pas enregistré, faut-t-il l'enregistrer ?");
	    if (wantCreateClient) {
		boolean clientCreated = this.createClient();
		if (clientCreated) {
		    this.openFacturationWindow();
	...
	if (this.dashboard.getClients().contains(this.client)) {
	    Alert alert = new Alert(AlertType.CONFIRMATION, "Supprimer " + this.client.getFullName() + " ?", ButtonType.YES, ButtonType.NO);
